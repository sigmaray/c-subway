...
static GameEntity make_obstacle(Lane lane, float positionZ,
  ...
  switch (variant) {
    case OBSTACLE_BARRIER:
      e.height = 1.55f;
      e.width = GAME_CONFIG.laneWidth * 0.88f;
      e.depth = 0.55f;
      e.baseY = 0.0f;
      break;
    ...
    case OBSTACLE_OVERHEAD:
      e.height = 1.05f;
      e.width = GAME_CONFIG.laneWidth * 0.92f;
      e.depth = 0.65f;
      e.baseY = 1.05f;
      break;
    case OBSTACLE_CRATE:
...
static uint32_t spawn_jump_row(WorldSegment *seg, float rowZ,
  ...
  int maxExclusive = 1 + (int)floor((double)(ctx.difficulty * 2.0f));
  IntResult countRoll;
  int i;
...
static uint32_t spawn_train_row(WorldSegment *seg, float rowZ,
  ...
  float movingChance = 0.35f + ctx.difficulty * 0.5f;
...
static uint32_t spawn_dual_train_row(WorldSegment *seg, float rowZ,
  ...
  for (i = 0; i < 3; i++) {
    ...
    }
    {
      ...
      float speed;
      seed = speedRoll.seed;
      movingRoll = chance(seed, 0.45f + ctx.difficulty * 0.35f);
      seed = movingRoll.seed;
...
static uint32_t spawn_mixed_row(WorldSegment *seg, float rowZ,
  ...
  trainRoll = chance(seed, 0.1f + ctx.difficulty * 0.16f);
  seed = trainRoll.seed;
  if (trainRoll.hit) {
    LanePick lanePick = pick_lane(seed);
    ChanceResult movingRoll;
  ...
  maxExclusive = 1 + (int)floor((double)(ctx.difficulty * 2.0f));
  if (maxExclusive > 2) {
    maxExclusive = 2;
  }
...
static uint32_t spawn_pattern(WorldSegment *seg, float startZ,
  ...
  float gapScale = 1.0f - ctx.difficulty * 0.42f;
  ...
  extraRows = (int)floor((double)(ctx.difficulty * 2.5f));
  ...
  for (row = 0; row < rowCount; row++) {
    float rowZ = startZ - 6.0f - (float)row * (8.5f * gapScale + 2.2f);
